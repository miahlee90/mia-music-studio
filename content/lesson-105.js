...
LESSON_CONTENT[105]={stackFigures:true,
  ...
  hook:{
    ...
    interact:{ type:"custom",
      mount:(container,fb)=>{
        ...
        container.querySelector(".hk-a").onclick=()=>{ [[57,60,64],[50,53,58],[52,56,59],[57,60,64]].forEach((row,i)=>row.forEach(m=>MFAudio.tone(m,.85,i*.9,.27))); setTimeout(()=>ch.style.display="",4*900+300); };
  ...
  mia:{
    hook:{ label:"the welcome",
      ...
      play:()=>{[[57,60,64],[50,53,58],[52,56,59],[57,60,64]].forEach((row,i)=>row.forEach(m=>MFAudio.tone(m,.8,i*.85,.27)));} },
